...
def init_llm(model_name: str = "llama-3.3-70b-versatile") -> ChatGroq:
    ...
    groq_api_key = os.getenv("GROQ_API_KEY_FOR_JSON")
    if not groq_api_key:
...
At first give a short answer based on the document of this type (json), then give a detailed and most accurate answer.
""")
...
def read_json_file(file_path: str) -> str:
    """
    Reads a JSON file and returns its content as a plain string.
    """
    ...
    # convert back to a clean readable string
    text = json.dumps(data, indent=2)

    logger.info("JSON file read successfully: %s (%d characters)", file_path, len(text))
    return text


# ── Step 2 : Load json into vectorstore ───────────────────────────────────────
def load_and_process_json(
    ...
    """
    Reads the json file, splits it into chunks, embeds them,
    and stores in ChromaDB. Returns the vectorstore.
    """
    # read raw json string
    json_text = read_json_file(file_path)
    ...
    logger.info("Splitting json into chunks...")
...
if __name__ == "__main__":
    ...
    JSON_PATH="../json_sample/sample.json"
 
    llm        = init_llm()
    vs         = load_and_process_json(JSON_PATH)
