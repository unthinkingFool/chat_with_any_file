...
# =============================================================================
VIDEO_URL   = "https://youtu.be/JAWSqX2fBvQ?si=ciPrh4vLGXXst48h"   #  URL
...
def download_audio(url: str, output_dir: str) -> str:
    ...
    Returns
    -------
    Path to the downloaded .mp3 file (str)
    """
    ...
    logger.info(" Download complete — '%s'", title)
...
def build_rag_chain(vectorstore: Chroma):
    ...
    """
    groq_api_key = os.getenv("GROQ_API_KEY_FOR_YOUTUBE_SCRAPPING")
    if not groq_api_key:
        raise ValueError(
...
# ─────────────────────────────────────────────────────────────────────────────
def main():
    print("\n🎬  Video RAG — Transcribe & Ask Questions About Any YouTube Video")
    print("=" * 70)

    # Use a temporary folder so audio files are cleaned up automatically
    with tempfile.TemporaryDirectory() as tmp_dir:

        # Step 1 — Download
        audio_path, video_title = download_audio(VIDEO_URL, tmp_dir)

        # Step 2 — Transcribe
        transcript = transcribe_audio(audio_path)

        # Step 3 — Index transcript in ChromaDB
        vectorstore = build_vectorstore(transcript, video_title)

        # Step 4 — Build LangChain RAG chain
        rag_chain, _ = build_rag_chain(vectorstore)

        # Step 5 — Interactive Q&A
        interactive_qa(rag_chain, transcript, video_title)
